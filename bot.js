...
// ─── CONFIG ───────────────────────────────────────────────
const GROUP_CHAT_ID = 'YOUR_GROUP_CHAT_ID';     // e.g. -100123456789
const SUPPORT_CHAT_ID = 'YOUR_SUPPORT_CHAT_ID'; // where new tickets get posted for admins
    ...
    if (tracker.count >= SPAM_LIMIT) {
      try {
        ...
        await bot.sendMessage(
          chatId,
          `🚫 @${msg.from.username || msg.from.first_name} has been muted for ${MUTE_DURATION}s due to spamming.`
        );
...
bot.onText(/^\/start/, (msg) => {
  bot.sendMessage(msg.chat.id, `👋 Welcome! I'm the group bot. Type /help to see commands.`);
});
...
bot.onText(/^\/warn(?:\s+@(\w+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  if (!await isAdmin(chatId, msg.from.id)) return bot.sendMessage(chatId, '⛔ Admins only.');
...
const TICKET_STEPS = ['name', 'email', 'phone', 'country', 'subject', 'coin', 'amount', 'description'];
...
bot.on('message', (msg) => {
  ...
  if (currentKey === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
    return bot.sendMessage(state.chatId, '⚠️ That doesn\'t look like a valid email. Try again:');
  }
  if (currentKey === 'phone' && !/^\+?[0-9]{7,15}$/.test(value)) {
    return bot.sendMessage(state.chatId, '⚠️ That doesn\'t look like a valid phone number. Try again (e.g. +2348012345678):');
  }
  if (currentKey === 'amount' && !/^[0-9]*\.?[0-9]+$/.test(value)) {
    return bot.sendMessage(state.chatId, '⚠️ Please enter a numeric amount (e.g. 0.5):');
  }
  if (currentKey === 'coin' && value.length > 10) {
    return bot.sendMessage(state.chatId, '⚠️ Please enter a valid coin symbol (e.g. BTC, ETH):');
  }
...
📝 Description: ${state.answers.description}
    ...
    bot.sendMessage(SUPPORT_CHAT_ID, `🚨 *New Support Ticket* ${ticketId}\n${summary}`, { parse_mode: 'Markdown' });

    // Persist to disk
    storage.addTicket(userId, { ticketId, status: 'open', timestamp, ...state.answers });
...
bot.onText(/^\/mytickets/, (msg) => {
  ...
  if (!tickets || tickets.length === 0) {
    return bot.sendMessage(msg.chat.id, 'You have no submitted tickets yet. Use /ticket to open one.');
  }
